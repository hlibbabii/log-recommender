...
class IoUtilsTest(unittest.TestCase):
    ...
    def test_dump_read_dict(self):
        sample = {'key': 'value'}
        io.dump_dict_into_2_columns(sample, PATH_TO_FILE)
        result = io.read_dict_from_2_columns(PATH_TO_FILE)
    ...
    def test_dump_read_dict_list_value(self):
        ...
        io.dump_dict_into_2_columns(sample, PATH_TO_FILE, val_type=list)
        result = io.read_dict_from_2_columns(PATH_TO_FILE, val_type=list)
        self.assertEqual(sample, result)
    ...
    def test_dump_read_dict_with_space_delim(self):
        sample = {'key': 'value1'}
        io.dump_dict_into_2_columns(sample, PATH_TO_FILE, delim='|')
        result = io.read_dict_from_2_columns(PATH_TO_FILE, delim='|')
        self.assertEqual(sample, result)

    def test_dump_read_list(self):
        sample = [['aa', 'bb', 'cc'], ['mm', 'hh', 'j']]
        io.dump_list(sample, PATH_TO_FILE)
        result = io.read_list(PATH_TO_FILE)
        self.assertEqual(sample, result)
