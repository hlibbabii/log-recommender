...
def get_classes_list(preprocessed_logs):
    ...
    classes = get_significant_words(first_word_freq_stats.keys(),
        lambda w, stats=first_word_freq_stats: stats[w]['__all_abs__'] > MIN_OCCURRENCES)
...
def select_logs_for_nn(preprocessed_logs, classes, MIN_OCCURRENCES):
    shuffle(preprocessed_logs)
    logs_for_nn = []
    d = dict([(cl, 0) for cl in classes])
    for log in preprocessed_logs:
        if log.first_word_cathegory in classes and d[log.first_word_cathegory] < MIN_OCCURRENCES:
            logs_for_nn.append(log)
            d[log.first_word_cathegory] += 1
    print("Logs selected for nn:")
    print("N classes: " + str(len(classes)))
    print("Logs from each class: " + str(MIN_OCCURRENCES))
    print("Logs in total: " + str(len(logs_for_nn)))
    shuffle(logs_for_nn)
    return logs_for_nn[100:], logs_for_nn[0:100]
...
if __name__ == "__main__":
    ...
    tstdata = ClassificationDataSet(n_input_nodes, n_hidden_layers, nb_classes=n_output_nodes)
    trndata = ClassificationDataSet(n_input_nodes, n_hidden_layers, nb_classes=n_output_nodes)
    for index, log in enumerate(logs_for_nn):
        words = get_interesting_words_from_logs(log, interesting_words_from_context)
        vector = build_vector(words, interesting_words_from_context)
        expected_class_label = get_label_by_class(log.first_word_cathegory, classes)
        if index % 4 == 3:
            tstdata.addSample(vector, [expected_class_label])
        else:
            trndata.addSample(vector, [expected_class_label])
    ...
    print(trndata['input'][0], trndata['target'][0], trndata['class'][0], logs_for_nn[0].first_word_cathegory)
