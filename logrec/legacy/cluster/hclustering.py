...
def run_hierarchical_clustering(log_vectors, first_words_vector, contexts, metric_function):
    ...
    for i, linkage_row in enumerate(linkage):
        ...
        print(i)
        print(str(node_index1) + " " + str(node_index2) + " " + str(linkage_row[2]) + " " + str(
            linkage_row[3].astype(int)) +
              " " + f1 + " + " + f2)
        print("===")
        if node_index1 < len(first_words_vector):
...
if __name__ == "__main__":
    ...
    parser.add_argument('--autoencode-dist-file', action='store',
                        default='../../AutoenCODE/out/rae/corpus.dist.matrix.csv')
